...
function workoutFor(key,create=false){
  if(!state.workouts[key]&&create){state.workouts[key]={date:key,planIds:[],items:[]};saveState(state)}
  return state.workouts[key];
}
function normalizeName(s){return s.toLowerCase().replace(/[^a-z0-9\u4e00-\u9fff]+/g,"")}
function similarName(name,id=""){
  const n=normalizeName(name);
  return state.exercises.find(x=>x.id!==id&&(normalizeName(x.name)===n||normalizeName(x.name).includes(n)||n.includes(normalizeName(x.name))));
}
...
function renderWorkout(){
  ...
  for(const category of sections){
    const entries=w.items.filter(x=>itemCategory(x)===category).sort((a,b)=>Number(isDone(a))-Number(isDone(b)));
...
function renderWorkoutItem(item){
  ...
  card.innerHTML=`<div class="item-head"><div><strong>${displayName}</strong><div class="muted">${labels[itemCategory(item)]}</div></div><div class="item-actions"><button class="secondary reference">Ref</button><button class="secondary remove">×</button></div></div><div class="item-body"></div>`;
  ...
  if(item.type==="cardio"){
    ...
    item.sets.forEach(set=>{
      const row=document.createElement("div");row.className="set-row";
      row.innerHTML=`<div class="unit-input"><input type="number" min="0" step="0.5" value="${set.weight||0}"><b>lb</b></div><input type="number" min="0" value="${set.reps||0}"><button class="${set.done?"":"secondary"}">${set.done?"✓":"○"}</button>`;
      row.children[0].querySelector("input").onchange=e=>{set.weight=Number(e.target.value)||0;saveState(state)};
      row.children[1].onchange=e=>{set.reps=Number(e.target.value)||0;saveState(state)};
...
function openPlan(plan=null){
  planDraft=plan?structuredClone({...plan,items:[...(plan.warmupAdditions||[]),...(plan.items||[])]}):{id:"",name:"",notes:"",items:[]};
  $("planDialogTitle").textContent=plan?"Edit plan":"Add plan";$("planId").value=planDraft.id;$("planName").value=planDraft.name;$("planNotes").value=planDraft.notes;$("planCategorySelect").value="warmup";intervalDraft=[10,10];populatePlanExerciseOptions();updatePlanFields();renderPlanIntervals();renderPlanDraft();$("planDialog").showModal();
}
function populatePlanExerciseOptions(){const category=$("planCategorySelect").value,used=new Set((planDraft.items||[]).map(x=>x.exerciseId)),available=state.exercises.filter(x=>!x.archived&&x.category===category&&!used.has(x.id));$("planExerciseSelect").innerHTML=available.length?available.map(x=>`<option value="${x.id}">${x.name}</option>`).join(""):'<option value="">No available exercise</option>'}
function updatePlanFields(){const c=$("planCategorySelect").value;$("planStrengthFields").classList.toggle("hidden",c==="cardio");$("planCardioFields").classList.toggle("hidden",c!=="cardio");$("planSets").value=c==="strength"?2:1;$("planReps").value=c==="strength"?12:(c==="flexibility"?30:6)}
function renderPlanIntervals(){$("planCardioIntervals").innerHTML=intervalDraft.map((m,i)=>`<div class="set-row"><div class="unit-input"><input data-i="${i}" type="number" min="1" value="${m}"><b>min</b></div><span></span><button type="button" class="secondary remove-plan-interval" data-i="${i}">×</button></div>`).join("");$("planCardioIntervals").querySelectorAll("input").forEach(x=>x.onchange=e=>intervalDraft[Number(e.target.dataset.i)]=Number(e.target.value)||1);$("planCardioIntervals").querySelectorAll(".remove-plan-interval").forEach(b=>b.onclick=()=>{if(intervalDraft.length>1){intervalDraft.splice(Number(b.dataset.i),1);renderPlanIntervals()}})}
function renderPlanDraft(){const grouped=sections.map(c=>{const its=(planDraft.items||[]).filter(x=>x.category===c);if(!its.length)return "";return `<h3 class="plan-items-heading">${labels[c]}</h3>`+its.map(x=>{const ex=exById(x.exerciseId),summary=x.type==="cardio"?`${(x.intervals||[]).join(" / ")} min`:`${x.sets} × ${x.reps}`;return `<div class="plan-item"><div class="section-head"><div><strong>${ex?.name||x.exerciseName||"Exercise"}</strong><div class="muted">${summary}</div></div><button type="button" class="secondary remove-plan-item" data-id="${x.id}">Remove</button></div></div>`}).join("")}).join("");$("planItemsList").innerHTML=grouped||'<p class="muted">No items.</p>';$("planItemsList").querySelectorAll(".remove-plan-item").forEach(b=>b.onclick=()=>{planDraft.items=planDraft.items.filter(x=>x.id!==b.dataset.id);renderPlanDraft();populatePlanExerciseOptions()})}
function addCurrentPlanItem(){const id=$("planExerciseSelect").value,ex=exById(id);if(!ex)return alert("No exercise available.");if(planDraft.items.some(x=>x.exerciseId===id))return alert("Exercise already added.");const item=ex.category==="cardio"?{id:crypto.randomUUID(),exerciseId:id,exerciseName:ex.name,category:"cardio",type:"cardio",intervals:[...intervalDraft]}:{id:crypto.randomUUID(),exerciseId:id,exerciseName:ex.name,category:ex.category,type:"exercise",sets:Number($("planSets").value)||1,reps:Number($("planReps").value)||1};planDraft.items.push(item);intervalDraft=[10,10];renderPlanDraft();populatePlanExerciseOptions();renderPlanIntervals()}
function renderLibrary(){
  ...
  for(const category of sections){
    ...
    for(const ex of list){
      ...
      card.querySelector(".delete-exercise").onclick=()=>{const pc=state.plans.filter(p=>(p.items||[]).some(i=>i.exerciseId===ex.id)).length,wc=Object.values(state.workouts).filter(w=>(w.items||[]).some(i=>i.exerciseId===ex.id)).length,detail=(pc||wc)?`\n\nUsed in ${pc} plan(s) and ${wc} workout date(s). Workout history will keep the exercise name.`:"";if(!confirm(`Delete "${ex.name}"?${detail}`))return;state.plans.forEach(p=>p.items=(p.items||[]).filter(i=>i.exerciseId!==ex.id));Object.values(state.workouts).forEach(w=>(w.items||[]).forEach(i=>{if(i.exerciseId===ex.id)i.exerciseName ||= ex.name}));state.exercises=state.exercises.filter(x=>x.id!==ex.id);persist()};
...
function renderAll(){renderHeader();renderWeek();renderWorkout();renderPlans();renderLibrary();renderProgress();renderSettings()}

function openExercise(ex=null){
  $("exerciseDialogTitle").textContent=ex?"Edit exercise":"Add exercise";$("exerciseId").value=ex?.id||"";$("exerciseName").value=ex?.name||"";$("exerciseCategory").value=ex?.category||"strength";$("exerciseMuscle").value=ex?.muscle||"";$("exerciseLink").value=ex?.link||"";$("exerciseNotes").value=ex?.notes||"";$("exercisePhoto").value="";$("exerciseDialog").showModal();
}
...
function updateAddMode(){
  ...
  $("addPlanPanel").classList.toggle("hidden",mode!=="plan");$("addExercisePanel").classList.toggle("hidden",mode!=="exercise");
}
function updateQuickExerciseFields(){
  const c=$("availableExercisesSelect").selectedOptions[0]?.dataset.category||"strength";
  $("strengthQuickFields").classList.toggle("hidden",c!=="strength");$("cardioQuickFields").classList.toggle("hidden",c!=="cardio");
}
...
function addPlanToWorkout(plan){const w=workoutFor(selectedDate,true);if(w.planIds.includes(plan.id))return false;const used=new Set(w.items.map(x=>x.exerciseId));for(const x of (plan.items||[]).filter(x=>!used.has(x.exerciseId))){const ex=exById(x.exerciseId),exerciseName=ex?.name||x.exerciseName||"Exercise";if(x.type==="cardio")w.items.push({id:crypto.randomUUID(),exerciseId:x.exerciseId,exerciseName,category:"cardio",type:"cardio",intervals:(x.intervals||[10]).map(minutes=>({minutes,done:false})),targetHr:""});else w.items.push({id:crypto.randomUUID(),exerciseId:x.exerciseId,exerciseName,category:x.category,type:"exercise",sets:Array.from({length:x.sets||1},()=>({weight:0,reps:x.reps||1,done:false}))})}w.planIds.push(plan.id);return true}
...
$("openAddWorkoutBtn").onclick=()=>{populateAddWorkout();updateAddMode();$("addWorkoutDialog").showModal()};
document.querySelectorAll('input[name="addMode"]').forEach(r=>r.onchange=updateAddMode);
$("availableExercisesSelect").onchange=updateQuickExerciseFields;
$("addQuickCardioIntervalBtn").onclick=()=>{intervalDraft.push(intervalDraft.at(-1)||10);renderQuickIntervals()};
$("addWorkoutForm").onsubmit=e=>{e.preventDefault();const mode=document.querySelector('input[name="addMode"]:checked').value,w=workoutFor(selectedDate,true);
  if(mode==="plan"){const p=state.plans.find(x=>x.id===$("availablePlansSelect").value);if(!p)return alert("No plan available.");if(!addPlanToWorkout(p))return alert("Plan already added.");}
  else{const id=$("availableExercisesSelect").value,ex=exById(id);if(!ex)return alert("No exercise available.");if(w.items.some(x=>x.exerciseId===id))return alert("Exercise already added.");
    if(ex.category==="cardio")w.items.push({id:crypto.randomUUID(),exerciseId:id,exerciseName:ex.name,category:"cardio",type:"cardio",intervals:intervalDraft.map(minutes=>({minutes,done:false})),targetHr:""});
    else w.items.push({id:crypto.randomUUID(),exerciseId:id,exerciseName:ex.name,category:"strength",type:"exercise",sets:Array.from({length:Number($("quickSets").value)||2},()=>({weight:0,reps:Number($("quickReps").value)||12,done:false}))});
  }
  $("addWorkoutDialog").close();persist();
};

$("addExerciseBtn").onclick=()=>openExercise();
$("exerciseForm").onsubmit=async e=>{e.preventDefault();const id=$("exerciseId").value,name=$("exerciseName").value.trim();if(!name)return;
  const exact=state.exercises.some(x=>x.id!==id&&normalizeName(x.name)===normalizeName(name));if(exact)return alert("This exercise already exists.");
  const similar=similarName(name,id);if(similar&&!confirm(`A similar exercise already exists: ${similar.name}\n\nSave anyway?`))return;
  const old=id?exById(id):null;let photo=old?.photo||"";if($("exercisePhoto").files[0])photo=await fileToDataUrl($("exercisePhoto").files[0]);
  const record={id:id||crypto.randomUUID(),name,category:$("exerciseCategory").value,muscle:$("exerciseMuscle").value.trim(),photo,link:$("exerciseLink").value.trim(),notes:$("exerciseNotes").value.trim(),archived:false};
  if(id)state.exercises[state.exercises.findIndex(x=>x.id===id)]=record;else state.exercises.push(record);
  $("exerciseDialog").close();persist();
};
$("librarySearch").oninput=renderLibrary;

$("addPlanBtn").onclick=()=>openPlan();
$("planCategorySelect").onchange=()=>{populatePlanExerciseOptions();updatePlanFields();intervalDraft=[10,10];renderPlanIntervals()};
$("addPlanCardioIntervalBtn").onclick=()=>{intervalDraft.push(intervalDraft.at(-1)||10);renderPlanIntervals()};
$("addPlanItemInlineBtn").onclick=addCurrentPlanItem;
$("planForm").onsubmit=e=>{e.preventDefault();planDraft.name=$("planName").value.trim();planDraft.notes=$("planNotes").value.trim();if(!planDraft.name)return;planDraft.id=planDraft.id||crypto.randomUUID();delete planDraft.warmupAdditions;const i=state.plans.findIndex(x=>x.id===planDraft.id);if(i>=0)state.plans[i]=structuredClone(planDraft);else state.plans.push(structuredClone(planDraft));$("planDialog").close();persist()};
$("exportBackupBtn").onclick=()=>downloadBackup(state);
$("restoreBackupInput").onchange=async e=>{const f=e.target.files[0];if(!f)return;try{state=JSON.parse(await f.text());saveState(state);renderAll();alert("Backup restored.")}catch{alert("Invalid backup file.")}};
